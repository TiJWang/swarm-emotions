class Robot {
  constructor(x, y, radius) {
    ...
    this.amplitude = random(5, 15); // reduce sine perturbation amplitude to show fear
    this.frequency = random(1, 2);   // reduce sine frequency
    this.velocity = createVector(random(-0.5, 0.5), random(-0.5, 0.5)); // reduce speed
  ...
  update(params, robots) {
    let speed = 0.01 * (params.P2 || 1); // p2 control speed, slower
    this.angle += speed;
    ...
    xOffset = constrain(xOffset, -this.radius / 3, this.radius / 3); // reduce perturbation amplitude
    ...
    let lerpFactor = 0.05 * (params.P3 || 1); // p3 control smoothness
    ...
    if (params.P4) {
      ...
      directionToCenter.setMag(0.01 * params.P4); // smaller
      this.position.add(directionToCenter);
    }

    // P5 
    if (this.isLeader && params.P5) {
      this.velocity.rotate(random(-0.02, 0.02) * params.P5); // smaller
