class Robot {
    constructor(x, y, radius) {
        ...
        this.amplitude = random(2, 5); // 减少正弦扰动的幅度，使机器人运动更加平稳
        this.frequency = random(2, 4);   // 调整正弦扰动的频率，使运动更加自然
        this.velocity = createVector(random(-1, 1), random(-1, 1)); // 减小初始速度范围，使运动更缓慢
        this.position = createVector(x + radius * cos(this.angle), y + radius * sin(this.angle));
        this.moving = true; 
        this.isLeader = random(1) < 0.2; // 保留领导者的概率
    }
  
    // 更新机器人位置，使其沿着带有轻微正弦扰动的圆形路径移动
    update(params, robots) {
        let speed = 0.02 * (params.P2 || 1); // 调整基础速度，使其更加平缓
        // speed = constrain(speed, 0.02, 0.05); // 保持速度在较低范围内
        ...
        let lerpFactor = 0.1 * (params.P3 || 1); // 调整运动的平滑度
        ...
        if (params.P1) {
            robots.forEach(other => {
                if (other !== this) {
                    ...
                    if (distance < params.P1 * 30) { // 保持适度的机器人间距
                        let repulsion = p5.Vector.sub(this.position, other.position);
                        repulsion.setMag(0.5); // 调整排斥力，使其更加温和
        ...
        if (params.P4) {
            ...
            directionToCenter.setMag(0.05 * params.P4); // 调整向中心的聚集力
            this.position.add(directionToCenter);
        }
  
        // P5 - 领导倾向
        if (this.isLeader && params.P5) {
            this.velocity.rotate(random(-0.05, 0.05) * params.P5); // 领导者的运动变化更平滑
            this.position.add(this.velocity);
        }
  
        // 边界吸引力，防止机器人离开中心区域
        let boundaryAttraction = 0.05; // 调整边界吸引力，使其更加柔和
        let distanceFromCenter = dist(this.position.x, this.position.y, width / 2, height / 2);
        if (distanceFromCenter > width / 3) { // 调整允许的中心距离
